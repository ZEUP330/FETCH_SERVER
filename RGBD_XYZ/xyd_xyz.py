...
class xyd_xyz():
    ...
    def __init__(self):
        ...
        self.cnn = torch.load("D:/GitDesktop/FETCH_SERVER/RGBD_XYZ/xy2xyz.pkl").cuda()
        # self.cnn.load_state_dict(torch.load("D:/GitDesktop/FETCH_SERVER/RGBD_XYZ/xy2xyz.pkl"))

    def load_model(self, model_path="xy2xyz.pkl"):
        self.cnn = torch.load(model_path)

    def test(self):
        self.load_model()
        t_xyd, t_xyz = random_read_()
        t_out = self.cnn(t_xyd).data
        print(t_xyz)
        print(t_out[0])
        ac = float(self.cal_acc(t_xyz.cpu().data.numpy(), t_out[0].cpu().data.numpy()))
        print("|test accuracy:%.4f" % ac)

    def cal_acc(self, a, b):
        sum1 = 0
        ...
        for epoch in range(EPOCH):
            for batch_idx, (xyd, xyz) in enumerate(self.train_loader):
                ...
                if batch_idx % 5 == 0:
                    ...
                    print(test_xyz)
                    print(test_out[0])
                    accuracy = float(self.cal_acc(test_xyz.cpu().data.numpy(), test_out[0].cpu().data.numpy()))
...
if __name__ == "__main__":
    ...
    # method.train()
    # method.test()
    print(method.detect(np.array([109, 90, 3])))
