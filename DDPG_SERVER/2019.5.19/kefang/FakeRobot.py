...
class Robot(object):
    ...
    def random_box_position(self):
        tmp = np.random.uniform(low=0, high=0.5, size=3)
        tmp[0] += 0.53
        tmp[1] -= 0.25
        tmp[2] = 0.75
    ...
    def test_step(self, action, var):
        ...
        # self.end_goal[0] = np.clip(np.random.normal(self.end_goal[0], var), -0.5, 0.5)
        # self.end_goal[1] = np.clip(np.random.normal(self.end_goal[1], var), -0.5, 0.5)
        # self.end_goal[2] = np.clip(np.random.normal(self.end_goal[2], var), -0.5, 0.5)
        self.end_goal[0] = np.random.normal(self.end_goal[0], var) % 0.4 + 0.53
        self.end_goal[1] = np.random.normal(self.end_goal[1], var) % 0.54 - 0.27
        self.end_goal[2] = np.random.normal(self.end_goal[2], var) % 0.1 + 0.75
        x = self.end_goal[0]
        y = self.end_goal[1]
        z = self.end_goal[2]
        # print(x, y)
