...
class DDPG(object):
    def __init__(self):
        ...
        self.Actor_eval = nn.DataParallel(self.Actor_eval, device_ids=self.device_ids)
        self.Actor_target = ANet().cuda(self.device_ids[0])
        self.Actor_target = nn.DataParallel(self.Actor_target, device_ids=self.device_ids)
        self.Critic_eval = CNet().cuda(self.device_ids[0])
        self.Critic_eval = nn.DataParallel(self.Critic_eval, device_ids=self.device_ids)
        self.Critic_target = CNet().cuda(self.device_ids[0])
        self.Critic_target = nn.DataParallel(self.Critic_target, device_ids=self.device_ids)
        self.ctrain = torch.optim.Adam(self.Critic_eval.parameters(), lr=LR_C)
        self.ctrain = nn.DataParallel(self.ctrain, device_ids=self.device_ids)
        self.atrain = torch.optim.Adam(self.Actor_eval.parameters(), lr=LR_A)
        self.atrain = nn.DataParallel(self.atrain, device_ids=self.device_ids)
    ...
    def save_model(self):
        model_number = self.get_file_number("Actor_eval_ddpg")
    ...
    def get_file_number(self, dir_name):
        a = 0
        file_dir = "/home/zhangtan/Desktop/DDPG_SERVER/models/"
