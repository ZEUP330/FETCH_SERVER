...
def rand_read_img():
    np_data_number = []
    for root, dirs, files in os.walk("/home/zhangtan/Desktop/images/rgb/"):
        np_data_number = files
    temp = np.random.randint(1, len(np_data_number))
    position = np_data_number[temp][:-4][1:-1].split(",")
    img_sample = cv2.imread("/home/zhangtan/Desktop/images/rgb/"+np_data_number[temp])
    dep_sample = np.load("/home/zhangtan/Desktop/images/dep/"+np_data_number[temp][:-4]+".npy")
...
def file_name(file_dir="/home/zhangtan/Desktop/DDPG_SERVER/memory"):
...
def load_model():
    ...
    while model_number > 0:
        try:
            ...
            rl.eval_net = torch.load("models/eval_dqn/" + str(model_number - 1) + ".pkl").cuda()
            rl.target_net = torch.load("models/target_dqn/" + str(model_number - 1) + ".pkl").cuda()
            break
...
if __name__ == "__main__":
    ...
    # rl.memory = np.load("/home/zhangtan/Desktop/ws/src/fetch_moveit_config/data/memory/"+str(file_name())+".npy")
    ...
    for i in range(1, MAX_EPISODES):
        ...
        while True:
            st += 1
            action = rl.choose_action([recent_end_goal, view_state])  # choose 时沿用之前的图像
            # print("max end goal:", max(endg))
            # recent_end_goal = action[0]
            recent_end_goal, r, done = robot.test_step(action, var)  # 执行一步
            rl.store_transition(observation, action, r, [recent_end_goal, view_state])  # 沿用之前的图像 RGBD
            ...
            if done or st >= MAX_EP_STEPS:
                ...
                break
            observation = [recent_end_goal, view_state]
        if i % 500 == 0:
            print("saving memory!!!")
            # np.save("/home/zhangtan/Desktop/ws/src/fetch_moveit_config/data/memory/"+str(rl.memory_counter), rl.memory)
            # print "sucess saved!!!"
